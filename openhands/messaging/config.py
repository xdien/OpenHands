...
class MessagingConfig(OpenHandsModel):
    ...
    enabled: bool = Field(
        default=False, 
        description="Enable messaging interface"
    )
    provider: MessagingProviderType = Field(
        default=MessagingProviderType.TELEGRAM,
        description="Messaging provider type"
    )
...
class TelegramConfig(OpenHandsModel):
    ...
    webhook_url: str | None = Field(
        default=None,
        description="Optional webhook URL. If not set, uses polling mode."
    )
    ...
    max_message_length: int = Field(
        default=4096,
        ge=1,
        description="Max message length before truncation"
    )
