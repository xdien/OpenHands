...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    def _get_agent_server_url(self, sandbox: SandboxInfo) -> str:
        ...
        Returns internal URL (direct port) for backend operations,
        or falls back to external URL if internal URL is not available.
        """
        exposed_urls = sandbox.exposed_urls
        assert exposed_urls is not None
        agent_server_exposed_url = next(
            exposed_url
            for exposed_url in exposed_urls
            if exposed_url.name == AGENT_SERVER
        )
        # Use internal_url for backend operations (execute bash, etc.)
        # Falls back to url if internal_url is not set (direct port mode)
        agent_server_url = (
            agent_server_exposed_url.internal_url or agent_server_exposed_url.url
        )
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        is_bailian_model = model and (
            model.startswith('bailian/')
            or any(
                model.lower().startswith(pattern.lower())
                for pattern in _BAILIAN_MODEL_PATTERNS
            )
        )
        if is_bailian_model:
