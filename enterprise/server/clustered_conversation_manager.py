...
# Exponential backoff for Redis connection errors (in seconds)
_REDIS_BACKOFF_INITIAL = 1.0
_REDIS_BACKOFF_MAX = 60.0
_REDIS_BACKOFF_JITTER = 0.1
...
class ClusteredConversationManager(StandaloneConversationManager):
    ...
    async def _redis_subscribe(self):
        ...
        Uses exponential backoff to avoid spamming logs when Redis is unavailable.
        """
        ...
        if redis_client is None:
            logger.warning(
                "Redis not available, _redis_subscribe will retry with backoff"
            )

        backoff = _REDIS_BACKOFF_INITIAL
            ...
            try:
                if redis_client is None or pubsub is None:
                    ...
                    if redis_client is None:
                        ...
                        backoff = min(backoff * 2, _REDIS_BACKOFF_MAX)
                        continue
                ...
                message = await pubsub.get_message(
                    ignore_subscribe_messages=True, timeout=_REDIS_POLL_TIMEOUT
                )
                if message:
                ...
                try:
                    ...
                    # Log with exponential backoff to avoid spam
                    logger.warning(f"error_reading_from_redis:{str(e)}")
                    await asyncio.sleep(backoff)
                    backoff = min(
                        backoff
                        * (
                            1
                            + _REDIS_BACKOFF_JITTER
                            + random.random() * _REDIS_BACKOFF_JITTER
                        ),
                        _REDIS_BACKOFF_MAX,
                    )
                    # Reset pubsub to retry connection
                    redis_client = None
                    pubsub = None
                except RuntimeError:
    ...
    async def _update_state_in_redis_task(self):
        """Task to periodically update conversation state in Redis.
        ...
        while should_continue():
            try:
                ...
                if redis is None:
                    # Wait with exponential backoff before retrying
                    logger.debug(
                        "Redis not available for _update_state_in_redis_task, backing off"
                    )
                    await asyncio.sleep(backoff)
                    backoff = min(
                        backoff
                ...
                try:
                    asyncio.get_running_loop()
                    # Log at warning level with backoff to avoid spam
                    logger.warning("error_updating_redis_state", exc_info=True)
                    await asyncio.sleep(backoff)
                    backoff = min(backoff * 2, _REDIS_BACKOFF_MAX)
                except RuntimeError:
