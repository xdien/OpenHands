...
async def configure(user_id, model, api_key, base_url):
    async with a_session_maker() as session:
        ...
        if not member:
            ...
            print(
                'Hint: Make sure the user has successfully linked their Discord account first.'
            )
            return
...
if __name__ == '__main__':
    if len(sys.argv) < 4:
        ...
        print(
            'Example: python configure_llm.py 76baa890... openai/google/gemini-2.0-flash-001 sk-or-v1-...'
        )
        sys.exit(1)
