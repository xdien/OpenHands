...
async def reset_password(username, new_password):
    ...
    try:
        ...
        print(
            f"✅ Password successfully reset for user '{actual_username}' (ID: {user_id})"
        )
...
async def main():
    if len(sys.argv) < 3:
        print(
            'Usage: poetry run python enterprise/scripts/keycloak_manage_users.py <username_or_email> <new_password>'
        )
        return
